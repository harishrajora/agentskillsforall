 ...
 * from the agentskillsforall.com download API instead of cloning git repos.
 *
 * Flow:
 *   1. GitHub Trees API → discover SKILL.md locations
 *   2. raw.githubusercontent.com → fetch frontmatter to get skill names
 *   3. agentskillsforall.com/api/download → fetch full file contents from cached blob
 */
...
const DOWNLOAD_BASE_URL = process.env.SKILLS_DOWNLOAD_URL || 'https://www.agentskillsforall.com';
...
/**
 * Fetch a skill's full file contents from the agentskillsforall.com download API.
 ...
 *   4. Compute slugs and fetch full snapshots from agentskillsforall.com download API
 *
...
export async function tryBlobInstall(
  ...
  // 5. Fetch full snapshots from agentskillsforall.com download API in parallel
