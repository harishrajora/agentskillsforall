...
function buildSecurityLines(
  ...
  // Footer link
  lines.push('');
  lines.push(`${pc.dim('Details:')} ${pc.dim(`https://www.agentskillsforall.com/${source}`)}`);
...
export async function runAdd(args: string[], options: AddOptions = {}): Promise<void> {
  ...
  if (!source) {
    ...
    console.log(`    ${pc.cyan('npx agentskillshub-cli add')} ${pc.yellow('<source>')} ${pc.dim('[options]')}`);
    console.log();
    console.log(pc.dim('  Example:'));
    console.log(`    ${pc.cyan('npx agentskillshub-cli add')} ${pc.yellow('LambdaTest/agent-skills')}`);
    console.log();
    process.exit(1);
  }
  ...
  try {
    ...
    if (sourceOwner === 'openclaw' && !options.dangerouslyAcceptOpenclawRisks) {
      ...
      console.log();
      p.log.message(
        `If you understand the risks, re-run with:\n\n  ${pc.cyan(`npx agentskillshub-cli add ${source} --dangerously-accept-openclaw-risks`)}\n`
      );
    ...
    if (parsed.type === 'local') {
      ...
      const BLOB_ALLOWED_OWNERS = ['LambdaTest', 'harishrajora'];
  ...
  try {
    ...
    if (install) {
      ...
      try {
        ...
        await runAdd(['LambdaTest/agent-skills'], {
          skill: ['find-skills'],
          global: true,
          yes: true,
        ...
        p.log.message(pc.dim('  npx agentskillshub-cli add LambdaTest/agent-skills@find-skills -g -y --all'));
      }
    } else {
      // User declined - dismiss the prompt
      await dismissPrompt('findSkillsPrompt');
      p.log.message(
        pc.dim('You can install it later with: npx agentskillshub-cli add LambdaTest/agent-skills@find-skills')
      );
    }
  } catch {
