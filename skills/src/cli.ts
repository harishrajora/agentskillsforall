...
const LOGO_LINES = [LOGO_LINE1, LOGO_LINE2, LOGO_LINE3];
...
function showBanner(): void {
  ...
  console.log();
  console.log(
    `  ${DIM}$${RESET} ${TEXT}npx agentskillshub-cli add ${DIM}<package>${RESET}        ${DIM}Add a new skill${RESET}`
  );
  console.log(
    `  ${DIM}$${RESET} ${TEXT}npx agentskillshub-cli remove${RESET}               ${DIM}Remove installed skills${RESET}`
  );
  console.log(
    `  ${DIM}$${RESET} ${TEXT}npx agentskillshub-cli list${RESET}                 ${DIM}List installed skills${RESET}`
  );
  console.log(
    `  ${DIM}$${RESET} ${TEXT}npx agentskillshub-cli find ${DIM}[query]${RESET}         ${DIM}Search for skills${RESET}`
  );
  console.log();
  console.log(
    `  ${DIM}$${RESET} ${TEXT}npx agentskillshub-cli update${RESET}               ${DIM}Update installed skills${RESET}`
  );
  console.log();
  console.log(
    `  ${DIM}$${RESET} ${TEXT}npx agentskillshub-cli experimental_install${RESET} ${DIM}Restore from skills-lock.json${RESET}`
  );
  console.log(
    `  ${DIM}$${RESET} ${TEXT}npx agentskillshub-cli init ${DIM}[name]${RESET}          ${DIM}Create a new skill${RESET}`
  );
  console.log(
    `  ${DIM}$${RESET} ${TEXT}npx agentskillshub-cli experimental_sync${RESET}    ${DIM}Sync skills from node_modules${RESET}`
  );
  console.log();
  console.log(`${DIM}try:${RESET} npx agentskillshub-cli add LambdaTest/agent-skills`);
  console.log();
  console.log(`Discover more skills at ${TEXT}https://www.agentskillsforall.com/${RESET}`);
  console.log();
}

function showHelp(): void {
  console.log(`
${BOLD}Usage:${RESET} agentskillshub-cli <command> [options]

${BOLD}Manage Skills:${RESET}
  add <package>        Add a skill package (alias: a)
                       e.g. LambdaTest/agent-skills
...
${BOLD}Examples:${RESET}
  ${DIM}$${RESET} agentskillshub-cli add LambdaTest/agent-skills
  ${DIM}$${RESET} agentskillshub-cli add LambdaTest/agent-skills -g
  ${DIM}$${RESET} agentskillshub-cli add LambdaTest/agent-skills --agent claude-code cursor
  ${DIM}$${RESET} agentskillshub-cli add LambdaTest/agent-skills --skill pr-review commit
  ${DIM}$${RESET} agentskillshub-cli remove                        ${DIM}# interactive remove${RESET}
  ${DIM}$${RESET} agentskillshub-cli remove web-design             ${DIM}# remove by name${RESET}
  ${DIM}$${RESET} agentskillshub-cli rm --global frontend-design
  ${DIM}$${RESET} agentskillshub-cli list                          ${DIM}# list project skills${RESET}
  ${DIM}$${RESET} agentskillshub-cli ls -g                         ${DIM}# list global skills${RESET}
  ${DIM}$${RESET} agentskillshub-cli ls -a claude-code             ${DIM}# filter by agent${RESET}
  ${DIM}$${RESET} agentskillshub-cli ls --json                      ${DIM}# JSON output${RESET}
  ${DIM}$${RESET} agentskillshub-cli find                          ${DIM}# interactive search${RESET}
  ${DIM}$${RESET} agentskillshub-cli find typescript               ${DIM}# search by keyword${RESET}
  ${DIM}$${RESET} agentskillshub-cli update
  ${DIM}$${RESET} agentskillshub-cli update my-skill             ${DIM}# update a single skill${RESET}
  ${DIM}$${RESET} agentskillshub-cli update -g                    ${DIM}# update global skills only${RESET}
  ${DIM}$${RESET} agentskillshub-cli experimental_install            ${DIM}# restore from skills-lock.json${RESET}
  ${DIM}$${RESET} agentskillshub-cli init my-skill
  ${DIM}$${RESET} agentskillshub-cli experimental_sync              ${DIM}# sync from node_modules${RESET}
  ${DIM}$${RESET} agentskillshub-cli experimental_sync -y           ${DIM}# sync without prompts${RESET}

Discover more skills at ${TEXT}https://www.agentskillsforall.com/${RESET}
`);
}

function showRemoveHelp(): void {
  console.log(`
${BOLD}Usage:${RESET} agentskillshub-cli remove [skills...] [options]
...
${BOLD}Examples:${RESET}
  ${DIM}$${RESET} agentskillshub-cli remove                           ${DIM}# interactive selection${RESET}
  ${DIM}$${RESET} agentskillshub-cli remove my-skill                   ${DIM}# remove specific skill${RESET}
  ${DIM}$${RESET} agentskillshub-cli remove skill1 skill2 -y           ${DIM}# remove multiple skills${RESET}
  ${DIM}$${RESET} agentskillshub-cli remove --global my-skill          ${DIM}# remove from global scope${RESET}
  ${DIM}$${RESET} agentskillshub-cli rm --agent claude-code my-skill   ${DIM}# remove from specific agent${RESET}
  ${DIM}$${RESET} agentskillshub-cli remove --all                      ${DIM}# remove all skills${RESET}
  ${DIM}$${RESET} agentskillshub-cli remove --skill '*' -a cursor      ${DIM}# remove all skills from cursor${RESET}
...
`;
  ...
  console.log(
    `  ${DIM}GitHub:${RESET}  Push to a repo, then ${TEXT}npx agentskillshub-cli add <owner>/<repo>${RESET}`
  );
  console.log(
    `  ${DIM}URL:${RESET}     Host the file, then ${TEXT}npx agentskillshub-cli add https://example.com/${displayPath}${RESET}`
  );
  console.log();
  console.log(`Browse existing skills for inspiration at ${TEXT}https://www.agentskillsforall.com/${RESET}`);
  console.log();
}
...
function printSkippedSkills(skipped: SkippedSkill[]): void {
  ...
  for (const [source, skills] of grouped) {
    ...
    console.log(`    ${DIM}To update: ${TEXT}npx agentskillshub-cli add ${source} -g -y${RESET}`);
  }
}
...
async function updateGlobalSkills(
  ...
  if (skillNames.length === 0) {
    if (!skillFilter) {
      ...
      console.log(`${DIM}Install skills with${RESET} ${TEXT}npx agentskillshub-cli add <package> -g${RESET}`);
    }
...
async function updateProjectSkills(
  ...
  if (projectSkills.length === 0) {
    if (!skillFilter) {
      ...
      console.log(
        `${DIM}Install project skills with${RESET} ${TEXT}npx agentskillshub-cli add <package>${RESET}`
      );
    }
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log(`Run ${BOLD}agentskillshub-cli --help${RESET} for usage.`);
  }
}
