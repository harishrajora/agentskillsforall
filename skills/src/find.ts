...
const SEARCH_API_BASE = process.env.SKILLS_API_URL || 'https://www.agentskillsforall.com';
...
${DIM}  1) npx agentskillshub-cli find [query]${RESET}
${DIM}  2) npx agentskillshub-cli add <owner/repo@skill>${RESET}`;
  ...
  if (query) {
    ...
    console.log(`${DIM}Install with${RESET} npx agentskillshub-cli add <owner/repo@skill>`);
    console.log();
    ...
    for (const skill of results.slice(0, 6)) {
      ...
      console.log(`${DIM}└ https://www.agentskillsforall.com/${skill.slug}${RESET}`);
      console.log();
    }
    return;
  }
  ...
  if (info && (await isRepoPublic(info.owner, info.repo))) {
    console.log(
      `${DIM}View the skill at${RESET} ${TEXT}https://www.agentskillsforall.com/${selected.slug}${RESET}`
    );
  } else {
    console.log(`${DIM}Discover more skills at${RESET} ${TEXT}https://www.agentskillsforall.com${RESET}`);
  }
